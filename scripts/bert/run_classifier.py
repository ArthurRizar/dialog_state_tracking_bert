...
class ECommerceClassificationProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            try:
                ...
                '''
                text_a = []
                #start = max(1, len(line) - 10)
                start = 1
                for item in line[start: -1]:
                    text_a.append(tokenization.convert_to_unicode(item.replace(' ', '')))
...
def convert_single_dst_example(ex_index, example, label_list, max_seq_length,
                                                     tokenizer):
    ...
    tokens_a = []
    for item in example.text_a:
        cur_tokens = tokenizer.tokenize(item)
        tokens_a.append(cur_tokens)

    tokens_b = tokenizer.tokenize(example.text_b)

    _truncate_seq_pair_for_dst(tokens_a, tokens_b, max_seq_length - 2)

    tokens = []
    segment_ids = []
    tokens.append("[CLS]")
    segment_ids.append(0)
    for cur_tokens in tokens_a:
        tokens.extend(cur_tokens)
        segment_ids.extend([0] * (len(cur_tokens) + 1))
        tokens.append("[SEP]")
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_dst_example(ex_index, example, label_list,
...
def _truncate_seq_pair_for_dst(tokens_a, tokens_b, max_length):
    while True:
        counts = [len(item) for item in tokens_a]
        total_length = sum(counts) + len(counts) + len(tokens_b)
        if total_length <= max_length:
            break
        if len(tokens_a) >= 2:
            tokens_a.pop(0)
            ...
            if len(tokens_a[0]) > len(tokens_b):
                tokens_a[0].pop()
            else:
                tokens_b.pop()
...
def print_metrics(record_file, predictions, num_features, seq_length, candidate_num=10, name='valid'):
...
def predict_metrics(labels, predictions, candidate_num=10):
    ...
    scores = [prediction['probabilities'][1].tolist() for prediction in predictions]
    ...
    for i, (s, l) in enumerate(zip(scores, labels)):
        if i % candidate_num == 0:
            data = []
        data.append((float(s), int(l)))

        if i % candidate_num == candidate_num - 1:
            total_num += 1
...
def main(_):
    ...
    processors = {
            "test": ECommerceClassificationProcessor
    }
    ...
    if FLAGS.do_predict:
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, "test_predict.results")
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            result = estimator.predict(input_fn=predict_input_fn)
            metrics = print_metrics(predict_file, result, num_actual_predict_examples, FLAGS.max_seq_length, name='test')
            writer.write("test set: MAP {} MRR {} Precision@1 {} Recall@1 {} Recall@2 {} Recall@5 {}\n".format(*metrics))
