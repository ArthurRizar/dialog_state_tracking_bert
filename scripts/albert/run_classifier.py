...
def convert_single_dst_example(ex_index, example, label_list, max_seq_length,
    ...
    _truncate_seq_pair_for_dst(tokens_a, tokens_b, max_seq_length - 2 - len(tokens_a))
...
def _truncate_seq_pair_for_dst(tokens_a, tokens_b, max_length):
    chunk = tokens_a + [tokens_b]
    while True:
        counts = [len(item) for item in chunk]
...
def print_metrics(record_file, predictions, num_features, seq_length, candidate_num=10):
    ...
    print("valid set: MAP {} MRR {} Precision@1 {} Recall@1 {} Recall@2 {} Recall@5 {}".format(*metrics))
    return metrics
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        output_eval_file = os.path.join(FLAGS.output_dir, "eval.results")
    ...
    if FLAGS.do_predict:
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, "predict.results")
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            result = estimator.predict(input_fn=predict_input_fn)
            metrics = print_metrics(predict_file, result, num_actual_predict_examples, FLAGS.max_seq_length)
            writer.write("valid set: MAP {} MRR {} Precision@1 {} Recall@1 {} Recall@2 {} Recall@5 {}\n".format(*metrics))
