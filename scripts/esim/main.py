...
flags.DEFINE_float("learning_rate", 4e-4, "The initial learning rate for Adam.")
flags.DEFINE_float("clip_c", 10., "Gradient clipping threshold.")

flags.DEFINE_string('dataset', 'ubuntu', 'dataset')
...
def predict_metrics(sess, cost_op, probability_op, iterator, candidate_num=10):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for eidx in range(FLAGS.max_train_epochs):
            ...
            valid_metrics, valid_scores = predict_metrics(
                sess, cost_op, probability_op, valid, candidate_num=candidate_num)
